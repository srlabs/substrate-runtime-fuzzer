...
//!
//! ## Genesis context
//! - All five accounts `[0;32]`…`[4;32]` start with `ENDOWMENT = 1 << 60` (see `main.rs`).
//! - `ExistentialDeposit = 1 CENTS = UNITS / 30_000`.
...
// ─── constants ───────────────────────────────────────────────────────────────
// Mirrors `rococo_runtime_constants::currency` (cannot import: rococo runtime
// dep is `default-features = false` and the constants module is not exposed).

const UNITS: u128 = 1_000_000_000_000;
const CENTS: u128 = UNITS / 30_000;
const ENDOWMENT: u128 = 1 << 60; // matches main.rs genesis (1 << 60 per account)
const ED: u128 = CENTS; // ExistentialDeposit
...
fn build_seeds() -> Vec<(String, Vec<u8>)> {
    ...
    //
    // Basic transfer: account(0) sends 1 UNIT to account(1).
    // =========================================================================
    {
        let mut data = Vec::new();
        data.extend(transfer(0, 1, UNITS));
    ...
    {
        let mut data = Vec::new();
        // Leave 1 token behind – below ED → source is reaped.
    ...
    // =========================================================================
    {
        ...
        data.extend(transfer(0, 1, 10 * UNITS));
    ...
    {
        let init = ENDOWMENT;
        let burn1 = UNITS; // leaves init - burn1, still alive
    ...
    // =========================================================================
    {
        ...
        data.extend(burn(0, UNITS, false));
    ...
    // =========================================================================
    {
        let mut data = Vec::new();
        for origin in 0u8..5 {
            data.extend(burn(origin, UNITS, false));
        }
    ...
    // =========================================================================
    {
        ...
        data.extend(burn(1, 5 * UNITS, false));
        // account(1) sends the rest back
        data.extend(transfer(1, 0, 5 * UNITS));
    ...
    // =========================================================================
    {
        ...
        data.extend(transfer_keep_alive(0, 2, 3 * UNITS));
