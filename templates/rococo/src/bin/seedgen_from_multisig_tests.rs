...
//! Fuzzer accounts all have `ENDOWMENT = 1 << 60`, so any deposit fits.
//!
...
//!
//! ### Inner call
//! `Balances::transfer_allow_death(dest = account(3), value = 1 UNIT)`
//! dispatched from the multisig account.  The multisig account is funded beforehand
//! by transferring 5 UNITS from account(0).
...
/// Rococo native unit (mirrors `rococo_runtime_constants::currency::UNITS`).
...
/// Standard inner call: transfer 1 UNIT from the multisig account to account(3).
